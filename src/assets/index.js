<<<<<<< HEAD
import bgFooter from "./bg-footer.svg";
import facebook from "./facebook-icon.png";
import heroImage from "./heroImage.png";
import twitter from "./twitter-icon.png";
import synkifyText from "./Synkify-Text.png";
import logo from "./logo.png";
import background from "./bg.png";
import google from "./google.png";

export { bgFooter, facebook, heroImage, twitter, synkifyText, logo, background, google };
=======
import bgFooter from './bg-footer.svg'
import facebook from './facebook-icon.png'
import heroImage from './heroImage.png'
import twitter from './twitter-icon.png'
import synkifyText from './Synkify-Text.png'
import logo from './logo.png'
import background from './bg.png'
import google from './google.png'
import heroDisplay from './hero-display.png'
import featuresImage from "./features.png"
import message from "./message.png"
import audience from "./audience.png"
import getStarted from "./get--started.png"
import connect from "./connect.png"
import audience1 from "./audience-1.png"
import audience2 from "./audience-2.png"
// import audience3 from "./audience-3.png"


export {bgFooter, facebook, heroDisplay, featuresImage, heroImage, twitter, synkifyText, logo, background, google, message, connect, audience, getStarted, audience1, audience2}
>>>>>>> fd099b268c66ccb3ca1bbc0248d93ae10e30afb9
